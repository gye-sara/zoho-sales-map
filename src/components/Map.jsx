...
export default function Map({ fianzas, stats, filtersOpen }) {
  ...
  useEffect(() => {
    ...
    mapObj.current = L.map(mapRef.current, {
      ...
      zoom: 6,
      zoomControl: false, // lo añadimos manualmente para controlarlo
    });
      ...
      marker.bindPopup(`
        <div style="font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;width:260px;">
          <div style="background:#1a1a2e;padding:12px 14px;margin:-14px -20px 12px;display:flex;align-items:center;justify-content:space-between;border-radius:4px 4px 0 0;">
            ...
            <span style="background:${estadoBadgeColor};color:${estadoTextColor};font-size:10px;font-weight:600;padding:2px 7px;border-radius:20px;text-transform:uppercase;">${esc(f.estado_contrato)}</span>
          </div>
          ...
          <div style="display:flex;flex-direction:column;gap:6px;">
            <div style="display:flex;justify-content:space-between;">
              ...
              <span style="font-size:11px;color:#222;font-weight:500;max-width:150px;text-align:right;">${esc(f.nombre_inmobiliaria)}</span>
            </div>
            <div style="display:flex;justify-content:space-between;">
      ...
      <div style={{
        ...
        >
          <span style={{ fontSize:'12px', fontWeight:700, color:'#1a1a2e' }}>
            📊 {(stats?.total ?? 0).toLocaleString('es-ES')} pólizas
          </span>
